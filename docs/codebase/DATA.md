...
* Self profile stats aggregate Submission-owned rows scoped by `user_id`; `attempted_problems` is distinct submitted `problem_id` values regardless of status, while solved counts use current `submissions.status`. They do not aggregate historical `submission_attempts` or `submission_results`. The activity response is bounded to the most recent 365 UTC submission dates.
