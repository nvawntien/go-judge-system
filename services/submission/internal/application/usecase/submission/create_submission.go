...
func (uc *createSubmissionUseCase) Execute(ctx context.Context, claims auth.Claims, req dto.CreateSubmissionRequest) (dto.CreateSubmissionResponse, error) {
	...
	sub := entity.NewSubmission(req.ProblemID, claims.UserID, claims.Username, language, req.SourceCode)
	...
	return dto.CreateSubmissionResponse{ID: sub.ID, Status: string(sub.Status)}, nil
}
