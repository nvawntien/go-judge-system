...
// HandleNoBody: Used for APIs that do not receive input and return data.
func HandleNoBody[Res any](c *gin.Context, fn func(context.Context) (Res, error), successCode int) {
...
// HandleWithClaimsParamsAndBody: claims + URI params + JSON body -> data.
func HandleWithClaimsParamsAndBody[P any, B any, Res any](c *gin.Context, fn func(context.Context, auth.Claims, P, B) (Res, error), successCode int) {
	claims, ok := auth.GetClaims(c)
	if !ok {
		HandleError(c, NewAppError(CodeUnauthorized, "unauthorized", nil))
		return
