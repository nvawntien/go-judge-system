...
def compare(run_dirs: list[str | Path], output: str | Path) -> pd.DataFrame:
    ...
    for values in groups.values():
        ...
        if is_burst:
            ...
            # This is observed pipeline terminal completion, never Judge Core
            # throughput: ordinary harness raw data has no compile-excluded
            # execution phase timestamps.
            rates = [m["pipeline"]["terminal_throughput_per_sec"] for _, m in values]
        ...
        row = {"comparison_kind": "burst" if is_burst else "sustained", "configuration_label": f"{label}; {first.run.get('mode')} {subject}", "system_config_label": label, "system_config_fingerprint": _config_key(system), "system_config_series": _config_series(system, label), "run_ids": [d.run.get("run_id") for d, _ in values], "repetitions": len(values), "requested_rate": requested, "burst_size": burst_size,
               "accepted_rate_mean": accepted_ci["mean"], "pipeline_terminal_throughput_mean": throughput_ci["mean"], "pipeline_terminal_throughput_std": throughput_ci["std"], "pipeline_terminal_throughput_ci95_low": throughput_ci["ci95_low"], "pipeline_terminal_throughput_ci95_high": throughput_ci["ci95_high"], "pipeline_terminal_throughput_ci_method": throughput_ci["method"],
               # Deprecated aliases retain historical experiment JSON readers.
               "throughput_mean": throughput_ci["mean"], "throughput_std": throughput_ci["std"], "throughput_ci95_low": throughput_ci["ci95_low"], "throughput_ci95_high": throughput_ci["ci95_high"], "throughput_ci_method": throughput_ci["method"],
