...
var (
	ErrAssignRoleAlreadyAssigned = response.NewAppError(response.CodeConflict, "user already has the specified role", nil)
	ErrNewPasswordSameAsCurrent  = response.NewAppError(response.CodeBadRequest, "new password is the same as the current password", nil)
	...
	ErrIncorrectCurrentPassword  = response.NewAppError(response.CodeInvalidPassword, "incorrect current password", nil)
